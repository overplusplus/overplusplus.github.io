(function($){
  ...
  $.fn.twentytwenty = function(options) {
    ...
    return this.each(function() {
      ...
      if (options.pause_video_on_mouse_click && !options.is_mobile) {
        container.on("click", pauseVideosOnClick);
      }

      mobile_show_second = false;
      function mobile_toggle() {
        if (!mobile_show_second) {
          onHoverLeftMost();
        } else {
          onHoverRightMost();
        }
        mobile_show_second = !mobile_show_second;
      }

      if (options.hover_to_leftmost) {
        if (!options.is_mobile) {
          container.on("mouseenter", onHoverLeftMost);
          container.on("mouseleave", onHoverRightMost);
        } else {
          container.on("click", mobile_toggle);
        }
      }
